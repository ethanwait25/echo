...
async function insertParagraph(entryId: number, index: number, text: string) {
    const { error } = await supabase
        .from("paragraph")
        .insert({
            entry_id: entryId,
...
async function getParagraphById(pgId: number) {
    const { data, error } = await supabase
        .from("paragraph")
        .select()
        .eq("pg_id", pgId)
...
async function updateParagraphByIndex(entryId: number, index: number, text: string) {
    const { error } = await supabase
        .from("paragraph")
        .update({ text: text })
        .eq("entry_id", entryId)
